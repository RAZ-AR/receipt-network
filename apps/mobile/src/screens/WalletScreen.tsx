...
type Ledger = { balance: number; transactions: { id: string; amount: number; createdAt: string; receiptNumber: string | null; totalAmount: string | null }[] };
        ...
        <>
          <View style={styles.balWrap}>
            ...
            <NeoSurface radius={s(80)} style={styles.balCircle}>
              {ledger ? (
                <Text style={styles.balNum}>{ledger.balance.toLocaleString("sr-RS")}</Text>
              ) : failed ? (
                <Text style={styles.balNum}>—</Text>
              ) : (
          ...
          <View style={styles.pillRow}>
            <View style={styles.pill}>
              <Text style={styles.pillText}>
                {failed ? "Server nedostupan" : `${ledger?.transactions.length ?? 0} transakcija`}
              </Text>
            </View>
          </View>
          ...
          <NeoSurface radius={s(18)} style={styles.card}>
            ...
            {ledger && ledger.transactions.length === 0 ? (
              <Text style={styles.empty}>Još nema transakcija. Skeniraj prvi račun.</Text>
            ) : null}
            {ledger?.transactions.map((tx) => (
              <View key={tx.id} style={styles.row}>
                <LinearGradient colors={["#D6F7DB", "#A8E8B4"]} style={styles.icon}>
                  <Ionicons name="receipt-outline" size={s(17)} color="#3E6F58" />
                </LinearGradient>
                <View style={styles.mid}>
                  <Text style={styles.name}>{tx.receiptNumber ?? "Račun"}</Text>
                  <Text style={styles.sub}>
                    {new Date(tx.createdAt).toLocaleDateString("sr-RS")}
                    {tx.totalAmount ? ` · ${tx.totalAmount} RSD` : ""}
                  </Text>
                </View>
                <Text style={[styles.amount, { color: tx.amount > 0 ? "#3E9E76" : "#B0607F" }]}>
...
const styles = StyleSheet.create({
  ...
  empty: { fontFamily: fontFamily.regular, fontSize: s(12.5), color: colors.ink2, paddingVertical: s(14) },
